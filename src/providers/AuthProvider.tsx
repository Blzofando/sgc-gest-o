...
import { auth, db } from "@/app/lib/firebase"; // Mantendo seu caminho de importação
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      if (currentUser) {
        // Usuário logado, agora vamos verificar se ele está APROVADO no banco
        try {
          ...
          if (userSnap.exists() && userSnap.data().approved === true) {
            ...
            // Usuário existe, mas não tem aprovação (approved: false ou inexistente)
            setIsApproved(false);
            setUser(currentUser);
          }
        } catch (error) {
